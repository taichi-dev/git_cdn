...
async def test_lock(tmpdir, loop):
    reset_log()
...
async def test_lock_ex_sh(tmpdir, loop):
    reset_log()
    fn = str(tmpdir / "lock.lock")
    p1 = Future()
...
async def test_lock_ex_sh_sh(tmpdir, loop):
    reset_log()
    fn = str(tmpdir / "lock.lock")
    p1 = Future()
    task1 = locking_coroutine(fn, fcntl.LOCK_EX, p1, 1)
...
async def test_lock_ex_sh_cancel_wait(tmpdir, loop, wait_time):
    reset_log()
    fn = str(tmpdir / "lock.lock")
    p1 = Future()
    task1 = locking_coroutine(fn, fcntl.LOCK_EX, p1, 1)
...
async def test_monkey_lock(tmpdir, loop, num_times, with_cancel_monkey):
